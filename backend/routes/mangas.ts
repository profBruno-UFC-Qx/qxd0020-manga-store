...
router.put('/:id', expressjwt({ secret: SECRET_KEY, algorithms: ["HS256"] }), (req: JWTRequest, res) => {
    if(req.auth?.role === "admin") {
        const id = Number(req.params.id)
        const { data } = req.body
        const { title, number, price }  = JSON.parse(data)
...
router.delete('/:id', expressjwt({ secret: SECRET_KEY, algorithms: ["HS256"] }), (req: JWTRequest, res) => {
    if(req.auth?.role === "admin") {
        const id = Number(req.params.id)
